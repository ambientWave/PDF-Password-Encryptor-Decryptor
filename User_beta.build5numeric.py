# -> todo 17/11/2022 11h, 8.5h, 6.5h, 4h, 10m, 2h, from 07:50 am to 10:20 am, set TestString_Bytes_Count to 6, set TestString to 0x372f2f303532, 0x343937313134, 0x333338343534, 0x313434333132, 0x3130303036, 0x3030302f
#
#     3337256436985 change in TestString took 4   h
#     4402224300804 change in TestString took 4   h
# 13503102300518916 change in TestString took 2.5 h
#     3255450928126 change in TestString took 2.5 h
...
print(str(ID))
with open("User_beta.build5numeric.txt", "r") as log_file:
...
while (int(TestUser, 2).to_bytes(32, byteorder='big')) != USER:  # user passkey combination trial iterative loop
    try:
        ...
        if TestString_bytes[0] == 0x3a:
            TestString_Bytes_Count += 1
            TestString = 0x0
            for e in range(0, TestString_Bytes_Count):
                TestString += 0x30 * (0x100 ** e)
                TestString_bytes = TestString.to_bytes(TestString_Bytes_Count, byteorder='big')
        for b in range(-1, -TestString_Bytes_Count - 1, -1):
            if 0x39 >= TestString_bytes[b] >= 0x30:
                pass
            elif TestString_bytes[b] == 0x3a:
                TestString += (((TestString + (0x100 ** (-b))) - ((0xa) * (0x100 ** (-b-1)))) - TestString) # + (TestString_Bytes_Count + b"""
        ...
        with open("User_beta.build5numeric.txt", "w") as log_file:
    ...
    print("time taken for complete execution of program", elapsed_epoch_time)  # about 0.0312 s for each run. The number of possible combinations is number_of_characters ^ 96. Can we limit the range of characters to be between 0 and z?
