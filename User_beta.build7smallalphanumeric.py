# -> todo 16/11/2022 11h, 8.5h, 6.5h, 4h, 10m, 2h, from 12:30 pm to 8:10 am, set TestString_Bytes_Count to 5, set TestString to 0x316c326177, 0x3166793960, 0x3162356273, 0x313662716f, 0x3130303036, 0x3030302f
#
# 166409414 change in TestString took 2.5 h
#  96020503 change in TestString took 2.5 h
...
# This algorithm exhibits exponential growth and therefore runs in O(a^n) time where a = 34 and n = number of characters of password
import hashlib
...
print(str(ID))
with open("User_beta.build7smallalphanumeric.txt", "r") as log_file:
...
while (int(TestUser, 2).to_bytes(32, byteorder='big')) != USER:  # user passkey combination trial iterative loop
    try:
        ...
        for b in range(-1, -TestString_Bytes_Count - 1, -1):
            if (0x39 >= TestString_bytes[b] >= 0x30) or (0x7a >= TestString_bytes[b] >= 0x61):
                pass
            elif TestString_bytes[b] == 0x3a:
                TestString += (0x27) * (0x100 ** (-b-1))
            elif TestString_bytes[b] == 0x7b:
                TestString += (((TestString + (0x100 ** (-b))) - ((0x4c) * (0x100 ** (-b - 1)))) - TestString)  # + (TestString_Bytes_Count + b"""
        ...
        with open("User_beta.build7smallalphanumeric.txt", "w") as log_file:
